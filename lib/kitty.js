...
function generate(colors, opacity, mode, context) {
    ...
    const accent = colors.base0A || "#00ffff";
    ...
    let content = `# Generated by Quickshell Kitty Template
foreground            ${text}
background            ${surface}
cursor                ${primary}
...
`;
    ...
    const reloadCommand = `
        ...
        if [ -n "$SOCKETS" ]; then
            for socket in $SOCKETS; do
                kitty @ --to "unix:$socket" set-colors -a ~/.cache/quickshell/themes/kitty.conf >/dev/null 2>&1 || true
            done
    ...
    return {
        destination: "~/.cache/quickshell/themes/kitty.conf",
        content: content,
