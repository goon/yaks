...
Singleton {
    ...
    function _emitWorkspaces() {
        ...
        for (var i = 0; i < sets.length; i++) {
            var ws = sets[i];
            var id = i + 1; // 1-based index as stable ID
    ...
    function quit() {
        // No native quit for a generic compositor backend
        // The session is managed externally
        console.warn("Niri: quit() is not supported via the native WindowManager API.");
    }
